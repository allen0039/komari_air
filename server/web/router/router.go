...
func registerPublicRoutes(r *gin.Engine) {
	...
	r.GET("/api/public/agent/install.sh", public_api.ServeAgentInstallSH)
	r.GET("/api/public/agent/install.ps1", public_api.ServeAgentInstallPS1)
	r.GET("/api/public/agent/version", public_api.AgentVersion)
	r.GET("/api/public/agent/download/:os/:arch", public_api.DownloadAgent)
...
func registerAdminRoutes(r *gin.Engine) {
	...
	// plugins: 安装流程通过统一的分片上传接口，启停/列表/日志走 RPC2。
